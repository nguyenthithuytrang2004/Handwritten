...
class OCRApp:
    ...
    def setup_ui(self):
        ...
        ttk.Checkbutton(config_frame, text="Tự động tiền xử lý (recommended)", variable=self.auto_preprocess_var).grid(row=5, column=0, sticky=tk.W, pady=(8, 2))
        ttk.Checkbutton(config_frame, text="Deskew (tự động phát hiện nghiêng)", variable=self.deskew_var).grid(row=6, column=0, sticky=tk.W, pady=2)
        ttk.Checkbutton(config_frame, text="Binarize (chuyển ảnh nhị phân)", variable=self.binarize_var).grid(row=7, column=0, sticky=tk.W, pady=2)
        # Compare engines option
        self.compare_engines_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(config_frame, text="So sánh cả VietOCR và Tesseract để chọn kết quả tốt nhất", variable=self.compare_engines_var).grid(row=9, column=0, sticky=tk.W, pady=(8, 2))

        # Preview preprocessing button
        ttk.Button(config_frame, text="🔎 Xem trước tiền xử lý", command=self.preview_preprocessing).grid(row=10, column=0, pady=(8, 0), sticky=(tk.W, tk.E))
